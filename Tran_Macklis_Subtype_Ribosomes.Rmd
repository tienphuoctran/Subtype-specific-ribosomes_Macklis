...
#### In addition to using Accession number for cross-referencing datasets to annotate proteins, I also use primary gene name. This is because of multiple reasons: 1) Some sources have old outdated Uniprot accession, so it's easier to use gene names; 2) one source lists RAPs from human, so it's easier to use gene names for cross-species referencing. 3) Some gene names readily identify the protein function, e.g. eukaryotic translation initiation factors have EIF at the beginning of their gene name. 
...
```{r Protein_Detection_Pattern_rRNA_IP}
...
### Primary normalization approach: "Median-of-ratios"
...
normalized_imputed_plot <- plot_normalization(data_se)

# Show the plot
normalized_imputed_plot

# Save the plot
# ggsave("Plots/Fig2_Supplement_normalized_imputed.pdf", plot = normalized_imputed_plot, width = 5.64, height = 3.73, units = "in")
...
## Second normalization approach: to Average intensitiy of core ribosomal proteins (related to Figure S2B and C)

```{r RP_based_Normalization}
### Annotate which proteins are core structural ribosomal proteins
Geometricmean_output_RP <- Geometricmean_output %>% filter(Proteins %in% Ribosome_uniprot_ID_GeneNames$Entry)

### Average RP intensity per sample, result is a named vector double with name of sample
RP_Geometricmean_by_Sample <- apply(Geometricmean_output_RP[, c(2:10)], MARGIN = 2, FUN = mean, na.rm = TRUE)

### Average intensity of all RPs across all samples
AverageRP_allsamples <- mean(RP_Geometricmean_by_Sample)

### Normalization step: for each sample, divide the value of each protein by the sample's mean RP intensity. I then multiply the resulting value by a constant (the average RP intensity across all samples) to rescale the values back to a meaningful range for comparison with median-of-ratios normalization.

normalized_df_RP_based <- Geometricmean_output %>%
  mutate(across(starts_with("Sub") | starts_with("Cal"),
                ~ . / RP_Geometricmean_by_Sample[cur_column()] * AverageRP_allsamples))

### Subsequent steps of imputation and differential analysis are identical

### Filter proteins present in 3 to 5 samples of both subtypes for subsequent differential analysis
normalized_df_RP_lite <- normalized_df_RP_based %>% column_to_rownames(var = "Proteins")
normalized_df_RP_lite["SubTally"] <-  5- rowSums(is.na(normalized_df_RP_lite %>% select(contains("Sub")))) 
normalized_df_RP_lite["CalTally"] <-  4- rowSums(is.na(normalized_df_RP_lite %>% select(contains("Cal")))) 
normalized_df_RP_filtered <- normalized_df_RP_lite %>% filter(SubTally>=3 & CalTally>=3) %>% select(-contains("Tally"))

### Within-group imputation with knn
## Load MsCoreUtils package here in the script to avoid namespace conflicts with other packages, 
library(MsCoreUtils) 
RP_normalized_Sub_samples <- normalized_df_RP_filtered %>% select(contains("Sub"))
RP_imputed_Sub_samples <- MsCoreUtils::impute_knn(data.matrix(RP_normalized_Sub_samples) ,k = 2) %>% as.data.frame()
RP_normalized_Cal_samples <- normalized_df_RP_filtered %>% select(contains("Cal"))
RP_imputed_Cal_samples <- MsCoreUtils::impute_knn(data.matrix(RP_normalized_Cal_samples ) ,k = 2) %>% as.data.frame()
## Merge the two imputed dataframes and make sure the columns such CPN samples preceed SCPN samples
merged_norm_imputed_RP_df <- merge(RP_imputed_Cal_samples, RP_imputed_Sub_samples, by = 'row.names', all = TRUE)

### DEP
### Rename df columns to use DEP
colnames(merged_norm_imputed_RP_df) <- c("Accession",
                           "Sample.Cal.Rep1",
                           "Sample.Cal.Rep2",
                           "Sample.Cal.Rep3",
                           "Sample.Cal.Rep4",
                           ...
                           "Sample.Sub.Rep5")
### Requisite step of giving proteins unique names for DEP. We are using Accession only,
data_unique_RP <- make_unique(merged_norm_imputed_RP_df, "Accession", "Accession")
### Functions to create the make-se object in DEP
### Note that DEP automatically log2-transform abundances
data_se_RP <- make_se_parse(data_unique_RP, grep("Sample.", colnames(data_unique_RP)), mode = "delim", sep = ".Rep")

### Plot distribution of protein abundances (Figure S2B)
RP_normalized_imputed_dist <- plot_normalization(data_se_RP)

## Show the plot
RP_normalized_imputed_dist
### Save the plot
# ggsave("Plots/FigS2B_RP_Normalization.pdf", plot = RP_normalized_imputed_dist, width = 5.03, height = 3.56, units = "in")

### Test using limma: Note: DEP by default uses fdrtools to calculate adjusted p-values
### However, we will use the Benjamini-Hochberg method (implemented in base R) for our manuscript.
data_diff_all_contrasts_RP <- test_diff(data_se_RP, type = "all")
## Denote significant proteins (modify "alpha" ("fdrtools" calculated p.adj cut off) and "lfc" as needed)
dep_RP <- add_rejections(data_diff_all_contrasts_RP, alpha = 0.1, lfc = log2(0))

### Using base p.adjust BH method to calculate adjusted p-values
df_wide_RP <- get_df_wide(dep_RP)

## Histogram of p-values, showing that it's appropriate to calculate adjusted p value
ggplot(df_wide_RP, aes(x=Cal_vs_Sub_p.val)) + geom_histogram(binwidth = 0.05)

## Multiple testing correction, adjusting p-values with p.adjust method "BH"
df_wide_BH_RP  <- df_wide_RP %>% mutate(Cal_vs_Sub_BH = p.adjust(Cal_vs_Sub_p.val, method = "BH")) 

### Match Accession with gene name
df_wide_BH_annotate_RP <- df_wide_BH_RP %>% 
  mutate(Accession = name) %>% 
  left_join(Group_analysis_Accession_name, by = "Accession") %>% 
  mutate(name = gene_name)

### Annotate proteins based on differential analysis result and whther they are ribosomal proteins
df_wide_BH_annotate_RP <- df_wide_BH_annotate_RP %>% filter(!is.na(Cal_vs_Sub_p.val)) %>%
  mutate(`Ribosomal_Protein` = name %in% Ribosome_uniprot_ID_GeneNames$`Gene Names` )%>% 
  mutate("Group" = case_when(
  Cal_vs_Sub_BH >= 0.1 & Ribosomal_Protein != TRUE  ~ "Non-RP (n.s.)",
  Cal_vs_Sub_BH >= 0.1 & Ribosomal_Protein == TRUE ~ "RP (n.s.)",
  Cal_vs_Sub_BH < 0.1 & Ribosomal_Protein == TRUE & Cal_vs_Sub_diff <0 ~ "RP enriched in SCPN", 
  Cal_vs_Sub_BH < 0.1 & Ribosomal_Protein == TRUE & Cal_vs_Sub_diff >0 ~ "RP enriched in CPN", 
  Cal_vs_Sub_BH < 0.1 & Ribosomal_Protein != TRUE & Cal_vs_Sub_diff <0 ~ "Non-RP enriched in SCPN",
  Cal_vs_Sub_BH < 0.1 & Ribosomal_Protein != TRUE & Cal_vs_Sub_diff >0 ~ "Non-RP enriched in CPN"
...
df_wide_BH_annotate_RP_pval <- df_wide_BH_annotate_RP %>% filter(!is.na(Cal_vs_Sub_p.val)) %>%
  mutate(`Ribosomal_Protein` = name %in% Ribosome_uniprot_ID_GeneNames$`Gene Names` )%>% 
  mutate("Group" = case_when(
  Cal_vs_Sub_p.val >= 0.01 & Ribosomal_Protein != TRUE  ~ "Shared non-RP",
  Cal_vs_Sub_p.val >= 0.01 & Ribosomal_Protein == TRUE ~ "Shared RP",
  Cal_vs_Sub_p.val < 0.01 & Ribosomal_Protein == TRUE & Cal_vs_Sub_diff <0 ~ "RP enriched in SCPN", 
  Cal_vs_Sub_p.val < 0.01 & Ribosomal_Protein == TRUE & Cal_vs_Sub_diff >0 ~ "RP enriched in CPN", 
  Cal_vs_Sub_p.val < 0.01 & Ribosomal_Protein != TRUE & Cal_vs_Sub_diff <0 ~ "Non-RP enriched in SCPN",
  Cal_vs_Sub_p.val < 0.01 & Ribosomal_Protein != TRUE & Cal_vs_Sub_diff >0 ~ "Non-RP enriched in CPN"
  )) 

### Volcano plot for visualization
## Using adjusted pval: Annotation of proteins found with primary normalization approach
ggplot(df_wide_BH_annotate_RP, mapping = aes(x= Cal_vs_Sub_diff, y= -log10(Cal_vs_Sub_BH), label = name)) + geom_point(aes(color = Group), alpha = 0.7) + geom_text_repel(data = filter(df_wide_BH_annotate_RP, grepl("Fau|Gucy1a1|Celf3",df_wide_BH_annotate_RP$name)), aes(label = toupper(name) ), size = 4,box.padding = unit(0.7, "lines"), point.padding = unit(0.1, "lines"), max.overlaps = 20, show.legend = FALSE) + labs(x="Log2(Fold Change)", y= "-Log10(Adjusted p-value)") + scale_colour_manual(values=c( "lightgray", "cornsilk4", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")) + scale_y_continuous(breaks = seq(0, 2, by = 0.5)) + theme_pubr(legend = "right")

## Using pval: with nominal cut off of 0.01, with annotation
RP_volcano <- ggplot(df_wide_BH_annotate_RP_pval, mapping = aes(x= Cal_vs_Sub_diff, y= -log10(Cal_vs_Sub_p.val), label = name)) + 
  geom_point(aes(color = Cal_vs_Sub_p.val <= 0.01), alpha = 0.7) + 
  geom_text_repel(data = filter(df_wide_BH_annotate_RP, grepl("Fau|Gucy1a1|Celf3",df_wide_BH_annotate_RP$name)),
                  aes(label = toupper(name), 
                      color = Cal_vs_Sub_p.val <= 0.01 & Cal_vs_Sub_diff > 0),
                  size = 4,box.padding = unit(0.5, "lines"), point.padding = unit(0.23, "lines"), max.overlaps = 20, show.legend = FALSE) + labs(x="Log2(Fold Change)", y= "-Log10(p-value)") + 
  scale_colour_manual(
    values = c("lightgray", "black"),
    labels = c("FALSE" = "p > 0.01", "TRUE" = "p \u2264 0.01"),
    name = NULL
  ) + 
  scale_y_continuous(breaks = seq(0, 3, by = 1)) + theme_pubr(legend = "right")

## Show the plot
RP_volcano
## Save pval plot
# ggsave("Plots/FigS2C_Volcano_RP_Normalization.pdf", plot = RP_volcano, width = 4.63, height = 3.56, units = "in")

### Output differential analysis results
df_wide_BH_annotate_output_RP <- df_wide_BH_annotate_RP %>% select(Accession, name, "Sub_1", "Sub_2", "Sub_3", "Sub_4", "Sub_5", "Cal_1", "Cal_2", "Cal_3", "Cal_4", Cal_vs_Sub_diff, Cal_vs_Sub_p.val, Cal_vs_Sub_BH)
colnames(df_wide_BH_annotate_output_RP) <- c("UniProt_Accession", "Gene", "SCPN_rRNA_Rep1", "SCPN_rRNA_Rep2", "SCPN_rRNA_Rep3", "SCPN_rRNA_Rep4", "SCPN_rRNA_Rep5", "CPN_rRNA_Rep1", "CPN_rRNA_Rep2", "CPN_rRNA_Rep3", "CPN_rRNA_Rep4", "Log2(CPN_rRNA/SCPN_rRNA)", "CPN_rRNA_vs_SCPN_rRNA_p.val", "CPN_rRNA_vs_SCPN_rRNA_p.adj")

## Write csv file
# write.csv(df_wide_BH_annotate_output_RP, file = "Tables/TableS6.csv", row.names = FALSE)
```

### Related to Fig 4: Subtype-specific gene expression of proteins that are exclusively detected in SCPN RCs and that have known interactions with components of ribosomes

```{r Transcriptomic_&_Translational_dataset_integration}
SCPN_Proteins_to_check <- "Cmas|Prkce|Snx27|S100a10|Dnaja1|Cct2|Pdia3|Pten|Mon2|Rtraf|Asns"
...
NanoRiboSeq_mRNA_annotated_plot <- ggplot(data = NanoRiboSeq_mRNA, aes(x=log10(baseMean), y= log2FoldChange))+ 
  geom_point(data = filter(NanoRiboSeq_mRNA, !is.na(`mRNA_sig`)),
             aes(color = `mRNA_sig`), alpha = 0.8, size = 2) + 
  scale_colour_manual(values=c("#ed6d52", "#4495d7", "#cccccc")) + 
  geom_text_repel(data = filter(NanoRiboSeq_mRNA,
                                grepl(SCPN_Proteins_to_check, NanoRiboSeq_mRNA$external_gene_name ) & `mRNA_sig` != "shared"),
...
# Show the plot
NanoRiboSeq_mRNA_annotated_plot

# Save the plot
# ggsave("Plots/Fig4A_mRNA_Annotated.pdf", plot = NanoRiboSeq_mRNA_annotated_plot, width = 6.5, height = 4.5, units = "in")

### Annotate p MA plot comparing ribosome-protected fragments between SCPN and CPN with names of proteins of interest
### Showing only transcripts with siginificant differences between subtypes
NanoRiboSeq_RPF_annotated_plot <- ggplot(data = NanoRiboSeq_RPF, aes(x=log10(baseMean), y= log2FoldChange)) + geom_point(data = filter(NanoRiboSeq_RPF, !is.na(`RP_sig`)),
             aes(color = `RP_sig`),
             shape=16,
             alpha = 0.8, size = 2 )+ 
  scale_colour_manual(values=c("#ed6d52", "#4495d7", "#cccccc")) + 
  geom_text_repel(data = filter(NanoRiboSeq_RPF,
                          grepl(SCPN_Proteins_to_check, NanoRiboSeq_RPF$external_gene_name ) & `RP_sig` != "n.s."),
  ...
  theme(
    ...
    legend.title = element_text(size = 12),  # Larger, bold legend title
    legend.text = element_text(size = 10)  # Larger legend text
  )
# Show the plot
NanoRiboSeq_RPF_annotated_plot

# Save the plot
# ggsave("Plots/Fig4B_RPF_Annotated.pdf", plot = NanoRiboSeq_RPF_annotated_plot, width = 6, height = 4, units = "in")
```
...
```
### Plotting PRKCE pulldown plots showing recovering of 28S and 18S rRNAs  (Figure 3C)
```{r PRKCE}
### Example Bioanalyzer electropherograms (Figure 3C)
Ctrl_IP_forPRKCE <- read_csv("PRKCE_IP_data/Ctrl_IP_for_PRKCE_Rep3.csv")
PRKCE_IP <- read_csv("PRKCE_IP_data/PRKCE_IP_Rep3.csv")

ggplot(Ctrl_IP_forPRKCE , mapping=aes(x=Time, y= Value)) +
  geom_line() +
  labs(x="Time (s)", y= "[FU]") +
  ylim(-2, 280) +
  theme_pubr()

ggplot(PRKCE_IP , mapping=aes(x=Time, y= Value)) +
  geom_line() +
  labs(x="Time (s)", y= "[FU]") +
  ylim(-2, 280) +
  theme_pubr()

### Plot of rRNA yield from paired IP samples
PRKCE_IP_rRNAs <- read_excel("PRKCE_IP_data/PRKCE_IP_rRNAs.xlsx")

PRKCE_IP_plot <- ggplot(PRKCE_IP_rRNAs, aes(x = Sample,  y = `28S_and_18S`, group = Replicate)) +
  geom_point()+
  geom_line(color = "gray60", linewidth = 0.8) +
  scale_x_discrete(limits = c("Ctrl IP", "PRKCE IP"),
                   labels = c("Ctrl IP\nN = 3", "PRKCE IP\nN = 3")) +
  scale_y_continuous(limits = c(0, 60), breaks = seq(0, 60, 10)) +
  labs(y = "28S and 18S rRNA recovery (ng)", x = NULL) +
  theme_pubr() 

# ggsave("Plots/Fig3D_PRKCE_rRNAs.pdf", plot = PRKCE_IP_plot, width = 2.2, height = 3.45, units = "in")
```